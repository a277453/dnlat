...
def cached_request(method: str, url: str, cache_enabled: bool = True, **kwargs):
    ...
    Args:
        ...
        cache_enabled: Whether to use cache for this request
        **kwargs: Additional arguments to pass to requests
        
    Returns:
        Response object (from cache or fresh request)
    """
    # Generate cache key
    cache_params = {
        'url': url,
...
def render_transaction_comparison():
    """
    Render transaction comparison analysis with source file filter and Transaction ID search
    """
...
def create_consolidated_flow_plotly(flow_data):
    """Create consolidated flow visualization with curved arrows and hover info"""
    import plotly.graph_objects as go
...
def render_counters_analysis():
    ...
    try:
        ...
        if disabled_count > 0:
            st.info(f"ℹ️ Counter analysis is only available for CIN/CI and COUT/GA transactions. {disabled_count} transaction(s) are not available for selection.")
