...
def _extract_journal_text(contents: dict, source_file: str) -> str:
    """
    Convert a {filename: text} journal dict into a single string for chat_service.

    Tries to match the transaction's source file stem first (e.g. "EJ_20260507"
    matches "EJ_20260507.log"). Falls back to concatenating all available content.
    """
    if not contents:
        return ""
    if source_file:
        ...
        for fname, text in contents.items():
            if Path(fname).stem == source_stem:
                return text or ""
    return "\n\n".join(t for t in contents.values() if t)
# ──────────────────────────────────────────────────────────────────────────────
...
async def chat_transaction(
    ...
    # Extract plain string content for chat_service (it expects str, not dict)
    txn_source_file = str(txn_data.get("Source File", "")).strip() if txn_data else ""
    ej_text  = _extract_journal_text(customer_journal_contents, txn_source_file)
    jrn_text = _extract_journal_text(all_jrn_contents,          txn_source_file)
