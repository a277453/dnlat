...
function CountersInner() {
  ...
  useEffect(() => {
    api.get('/get-matching-sources-for-trc', { timeout:30000 })
  ...
  const handleLoad = async () => {
    ...
    try {
      const r = await api.post('/get-counter-data', { transaction_id: selectedId, source_file: selSource }, { timeout:60000 });
      setCntData(r.data);
  ...
  // Get logical counters for selected per-txn row
  let logicalCounters = [];
  if (selPerTxn && cntData?.all_blocks) {
    const parts = selPerTxn.split(' | ');
    const dtStr = parts[0]; // "DD Month YYYY HH:MM:SS"
    try {
      const txnDt  = new Date(dtStr);
      const txnTs  = txnDt.getTime();
      cntData.all_blocks.forEach(blk => {
        if (!blk.time) return;
        const [h,m,s] = blk.time.split(':').map(Number);
        const blkDt = new Date(txnDt); blkDt.setHours(h,m,s,0);
        if (Math.abs(blkDt.getTime() - txnTs) <= 30000) {
          (blk.data || []).filter(c => c.Record_Type === 'Logical').forEach(c => {
            logicalCounters.push({
              'Name (PName)': c.UnitName,'Value (Val)': c.Val,'Cur': c.Cur,'Ini': c.Ini,
              'Retr': c.Retr,'Disp': c.Disp,'RCNT (Reject Count)': c.RCnt,
              'Pres': c.Pres,'Cnt': c.Cnt,'Status (St)': c.St,'NrPCU': c.No,
            });
          });
        }
      });
      // Deduplicate
      const seen = new Set();
      logicalCounters = logicalCounters.filter(r => {
        ...
        <div className="fade-in">
          ...
          {selPerTxn && (
            <div className="card fade-in mb-16">
              <div className="section-heading mb-4">Counters for: {selPerTxn.split(' | ')[1]}</div>
              <p style={{ color:'var(--text-muted)', fontSize:12, marginBottom:12 }}>{selPerTxn.split(' | ')[0]}</p>
              {logicalCounters.length > 0
                ? <>
                    <ThemedTable data={logicalCounters} height={320} />
                    <p style={{ color:'var(--text-muted)', fontSize:12, marginTop:8 }}>Showing {logicalCounters.length} unique counter record(s).</p>
                  </>
                : <div className="alert alert-info">No logical counters found for this transaction timeframe.</div>
              }
            </div>
          )}
